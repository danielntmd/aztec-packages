...
def selected_benchmarks(benchmarks: list[dict]) -> list[dict]:
...
def run_benchmark(
    ...
    for entry in selected_benchmarks(payload.get("benchmarks", [])):
...
def print_table(rows: dict[int, dict[str, float]], enabled: list[str]) -> None:
    ...
    for log_size in sorted(rows):
        row = rows[log_size]
        cpu = row.get("cpu_ms")
        bb = row.get("bb_e2e_ms")
        bb_backend = row.get("bb_e2e_backend_ms")
        v28_device = row.get("v28_device_ms")
        v4_device = row.get("v4_device_ms")
        values = [
            f"2^{log_size}",
            str(int(row["bb_c"])) if "bb_c" in row else "-",
            ...
            fmt_ms(v4_device),
            fmt_ratio(cpu / bb if cpu and bb else None),
            fmt_ratio(bb_backend / v28_device if bb_backend and v28_device else None),
            fmt_ratio(bb_backend / v4_device if bb_backend and v4_device else None),
        ]
    ...
    for log_size in sorted(rows):
        ...
        values = [
            f"2^{log_size}",
            fmt_ms(row.get("bb_e2e_preprocess_ms")),
            fmt_ms(row.get("bb_e2e_backend_ms")),
...
def main() -> int:
    ...
    parser.add_argument("--log-sizes", nargs="+", type=int, default=list(range(10, 21, 2)))
