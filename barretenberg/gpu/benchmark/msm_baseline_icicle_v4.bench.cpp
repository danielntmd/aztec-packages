...
void check_icicle(const icicle::eIcicleError error, const char* operation)
{
    if (error != icicle::eIcicleError::SUCCESS) {
        std::fprintf(stderr, "Icicle v4 %s failed: %s\n", operation, icicle::get_error_string(error));
        std::abort();
    }
}
...
void ensure_icicle_cuda()
{
    static const bool initialized = []() {
        ...
        check_icicle(icicle_load_backend(BB_GPU_ICICLE_V4_BACKEND_DIR, true), "load backend");
#else
...
{
    return env_u32("MSM_BENCH_C_OVERRIDE", msm_bench::auto_bits_per_slice(num_points));
}
...
msm_bench::Commitment to_cpu_point(const bn254::affine_t& point)
{
    ...
    return { msm_bench::Curve::BaseField(bb::numeric::uint256_t(limbs[0], limbs[1], limbs[2], limbs[3])),
             msm_bench::Curve::BaseField(bb::numeric::uint256_t(limbs[4], limbs[5], limbs[6], limbs[7])) };
}

RawInput make_raw_input(const msm_bench::BenchInput& input)
{
    const auto scalars = input.polynomial.coeffs();
    const std::span<const msm_bench::Commitment> points = input.commitment_key.get_monomial_points();
    RawInput raw;
    ...
    for (size_t i = 0; i < scalars.size(); ++i) {
        raw.points.push_back({ to_raw_field(points[i].x), to_raw_field(points[i].y) });
    }
    return raw;
}
                                  ...
                                  const bool results_on_device)
{
    ...
    static icicle::ConfigExtension cuda_msm_ext = []() {
        ...
        ext.set(CudaBackendConfig::CUDA_MSM_IS_BIG_TRIANGLE, env_bool("ICICLE_MSM_BIG_TRIANGLE", false));
        ext.set(CudaBackendConfig::CUDA_MSM_LARGE_BUCKET_FACTOR, env_int("ICICLE_MSM_LARGE_BUCKET_FACTOR", 10));
        ext.set(CudaBackendConfig::CUDA_MSM_NOF_CHUNKS, env_int("ICICLE_MSM_NOF_CHUNKS", 0));
        return ext;
    }();
...
bn254::projective_t icicle_v4_backend_msm(const RawInput& raw, const uint32_t bits_per_slice)
{
    static_assert(sizeof(bn254::scalar_t) == sizeof(RawField));
    static_assert(sizeof(bn254::affine_t) == sizeof(RawAffine));
    icicle::MSMConfig config = make_msm_config(bits_per_slice, false, false, false, false, false);

    bn254::projective_t result{};
    check_icicle(icicle::msm(reinterpret_cast<const bn254::scalar_t*>(raw.scalars.data()),
                             reinterpret_cast<const bn254::affine_t*>(raw.points.data()),
                             static_cast<int>(raw.scalars.size()),
                             config,
                             &result),
                 "MSM");
    return result;
...
msm_bench::Commitment icicle_v4_msm(const msm_bench::BenchInput& input, const uint32_t bits_per_slice)
{
...
DeviceRawInput make_device_raw_input(const RawInput& raw, double& h2d_ms)
{
    ...
    h2d_ms = elapsed_ms([&]() {
        check_icicle(icicle_malloc(reinterpret_cast<void**>(&device.scalars), raw.scalars.size() * sizeof(RawField)),
                     "device scalar allocation");
        check_icicle(icicle_malloc(reinterpret_cast<void**>(&device.points), raw.points.size() * sizeof(RawAffine)),
                     "device point allocation");
        check_icicle(icicle_malloc(reinterpret_cast<void**>(&device.projective_result), sizeof(bn254::projective_t)),
                     "device result allocation");
        check_icicle(icicle_copy_to_device(device.scalars, raw.scalars.data(), raw.scalars.size() * sizeof(RawField)),
                     "H2D scalars");
        check_icicle(icicle_copy_to_device(device.points, raw.points.data(), raw.points.size() * sizeof(RawAffine)),
                     "H2D points");
    });
    return device;
}

void icicle_v4_device_backend_msm(const DeviceRawInput& raw, const uint32_t bits_per_slice)
{
    icicle::MSMConfig config = make_msm_config(bits_per_slice, true, false, true, false, true);
    check_icicle(icicle::msm(raw.scalars, raw.points, static_cast<int>(raw.count), config, raw.projective_result),
                 "device-resident MSM");
}

msm_bench::Commitment finalize_icicle_v4_device_result(const DeviceRawInput& raw)
{
    bn254::projective_t projective_result{};
    check_icicle(icicle_copy_to_host(&projective_result, raw.projective_result, sizeof(projective_result)),
                 "D2H result");
    return finalize_icicle_v4_result(projective_result);
}

void add_common_counters(benchmark::State& state, const uint32_t bits_per_slice, const PhaseTotals& totals)
{
...
void bench_icicle_v4_e2e(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        ...
        totals.preprocess_ms += elapsed_ms([&]() { raw = make_raw_input(*input); });
        totals.backend_ms += elapsed_ms([&]() { backend_result = icicle_v4_backend_msm(raw, bits_per_slice); });
        totals.postprocess_ms += elapsed_ms([&]() { result = finalize_icicle_v4_result(backend_result); });
...
void bench_icicle_v4_prepared_host(benchmark::State& state)
{
    ...
    add_common_counters(state, bits_per_slice, totals);
    bn254::projective_t actual_projective = icicle_v4_backend_msm(raw, bits_per_slice);
    const auto actual = finalize_icicle_v4_result(actual_projective);
    msm_bench::assert_equal("Icicle v4 prepared host", log_num_points, expected, actual);
}
...
void bench_icicle_v4_device_resident(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        totals.backend_ms += elapsed_ms([&]() { icicle_v4_device_backend_msm(device, bits_per_slice); });
        benchmark::DoNotOptimize(device.projective_result);
    }

    add_common_counters(state, bits_per_slice, totals);
    const auto actual = finalize_icicle_v4_device_result(device);
    msm_bench::assert_equal("Icicle v4 device resident", log_num_points, expected, actual);
}

BENCHMARK(bench_icicle_v4_e2e)
    ->Name("BN254/Baseline/IcicleV4/E2E")
    ->DenseRange(msm_bench::MIN_LOG_NUM_POINTS, msm_bench::MAX_LOG_NUM_POINTS, 2)
