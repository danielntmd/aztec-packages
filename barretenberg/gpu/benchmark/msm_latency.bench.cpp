...
constexpr int MAX_LOG_NUM_POINTS = 20;
constexpr std::array<int, 6> AGGREGATE_LOG_NUM_POINTS = { 10, 12, 14, 16, 18, 20 };
...
{
    const auto scalar_span = bb::PolynomialSpan<const Fr>{ 0, input.polynomial.coeffs() };
    const std::span<const Commitment> points = input.commitment_key.get_monomial_points();
    return bb::scalar_multiplication::pippenger_unsafe<Curve>(scalar_span, points);
}
                            ...
                            bb::gpu::bn254::msm_profile* profile = nullptr)
{
    ...
    bb::gpu::bn254::msm_raw_profiled(reinterpret_cast<const bb::gpu::bn254::fr_t*>(scalars.data()),
                                     scalars.size(),
...
{
    std::fprintf(stderr, "GPU MSM benchmark correctness check failed for n=2^%d\n", log_num_points);
    std::abort();
}

void assert_correctness(const BenchInput& input, const uint32_t bits_per_slice, const int log_num_points)
{
...
void assert_commit_correctness(const BenchInput& input, const int log_num_points)
{
    const Commitment expected = cpu_msm(input);
...
void add_profile_counters(benchmark::State& state, const bb::gpu::bn254::msm_profile& totals)
{
    ...
    state.counters["scan_offsets_ms"] = totals.scan_bucket_offsets_ms / iterations;
    ...
    state.counters["normal_accum_ms"] = totals.accumulate_normal_buckets_ms / iterations;
    state.counters["large_accum_ms"] = totals.accumulate_large_buckets_ms / iterations;
    ...
    state.counters["entries"] = benchmark::Counter(totals.total_entries / iterations);
    state.counters["active_buckets"] = benchmark::Counter(totals.active_buckets / iterations);
    state.counters["encoded_buckets"] = benchmark::Counter(totals.encoded_buckets / iterations);
    state.counters["bucket_threshold"] = benchmark::Counter(totals.large_bucket_threshold / iterations);
}

void add_profile(bb::gpu::bn254::msm_profile& totals, const bb::gpu::bn254::msm_profile& profile)
{
...
void bench_gpu_all_sizes_profiled(benchmark::State& state)
{
    ...
    std::array<std::unique_ptr<BenchInput>, AGGREGATE_LOG_NUM_POINTS.size()> inputs;
    ...
    std::array<bb::gpu::bn254::msm_profile, AGGREGATE_LOG_NUM_POINTS.size()> gpu_totals{};
    ...
    for (size_t i = 0; i < AGGREGATE_LOG_NUM_POINTS.size(); ++i) {
        const std::string prefix = "n" + std::to_string(AGGREGATE_LOG_NUM_POINTS[i]) + "_";
        const double cpu_ms = counter_average(cpu_totals_ms[i], iterations);
        const double gpu_ms = counter_average(gpu_totals[i].total_profiled_ms, iterations);
        const double h2d_ms = counter_average(gpu_totals[i].h2d_points_ms + gpu_totals[i].h2d_scalars_ms, iterations);
        const double accum_ms = counter_average(
            gpu_totals[i].accumulate_normal_buckets_ms + gpu_totals[i].accumulate_large_buckets_ms, iterations);

        state.counters[prefix + "c"] = benchmark::Counter(gpu_totals[i].bits_per_slice / iterations);
        ...
        state.counters[prefix + "split_ms"] = counter_average(gpu_totals[i].split_scalars_ms, iterations);
        state.counters[prefix + "sort_records_ms"] = counter_average(gpu_totals[i].sort_records_ms, iterations);
        state.counters[prefix + "accum_ms"] = accum_ms;
        state.counters[prefix + "reduce_ms"] = counter_average(gpu_totals[i].reduce_buckets_ms, iterations);
        state.counters[prefix + "final_ms"] = counter_average(gpu_totals[i].final_accumulation_ms, iterations);
        state.counters[prefix + "d2h_ms"] = counter_average(gpu_totals[i].d2h_result_ms, iterations);
        state.counters[prefix + "active_buckets"] = benchmark::Counter(gpu_totals[i].active_buckets / iterations);

        total_cpu_ms += cpu_ms;
        total_gpu_ms += gpu_ms;
        total_h2d_ms += h2d_ms;
        total_split_ms += counter_average(gpu_totals[i].split_scalars_ms, iterations);
        total_sort_records_ms += counter_average(gpu_totals[i].sort_records_ms, iterations);
        total_accum_ms += accum_ms;
        total_reduce_ms += counter_average(gpu_totals[i].reduce_buckets_ms, iterations);
        total_final_ms += counter_average(gpu_totals[i].final_accumulation_ms, iterations);
    ...
    for (size_t i = 0; i < AGGREGATE_LOG_NUM_POINTS.size(); ++i) {
        assert_correctness(*inputs[i], bits_per_slice[i], AGGREGATE_LOG_NUM_POINTS[i]);
    }
}

BENCHMARK(bench_cpu_single_msm)->Name("BN254/CPU/single_msm")->DenseRange(MIN_LOG_NUM_POINTS, MAX_LOG_NUM_POINTS, 2)->Unit(benchmark::kMillisecond);
...
BENCHMARK(bench_gpu_single_msm_profiled)
    ...
    ->DenseRange(MIN_LOG_NUM_POINTS, MAX_LOG_NUM_POINTS, 2)
    ->Unit(benchmark::kMillisecond);
BENCHMARK(bench_gpu_all_sizes_profiled)->Name("BN254/GPU/all_sizes_profiled")->Unit(benchmark::kMillisecond);
