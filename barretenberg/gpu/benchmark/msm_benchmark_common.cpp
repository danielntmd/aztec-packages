...
{
    auto& engine = bb::numeric::get_debug_randomness(true, SCALAR_SEED_BASE + num_points);
...
BenchInput::BenchInput(const size_t num_points)
    ...
    , polynomial(make_polynomial(num_points))
{}
...
void assert_equal(std::string_view backend,
                  ...
                  const Commitment& actual)
{
    if (actual != expected) {
        std::fprintf(stderr,
                     "%.*s MSM benchmark correctness check failed for n=2^%d\n",
