...
msm_bench::Commitment bb_gpu_msm(const msm_bench::BenchInput& input,
...
{
    if (const char* override = std::getenv("MSM_BENCH_C_OVERRIDE"); override != nullptr) {
...
void add_profile_counters(benchmark::State& state, const bb::gpu::bn254::msm_profile& totals)
{
    ...
    const double preprocess_ms = (totals.h2d_points_ms + totals.h2d_scalars_ms + totals.split_scalars_ms +
                                  totals.sort_records_ms + totals.encode_buckets_ms + totals.scan_bucket_offsets_ms +
                                  totals.build_bucket_jobs_ms + totals.sort_bucket_jobs_ms + totals.init_buckets_ms) /
                                 iterations;
    const double backend_ms = (totals.accumulate_normal_buckets_ms + totals.accumulate_large_buckets_ms +
                               totals.reduce_buckets_ms + totals.compose_windows_ms +
                               totals.final_accumulation_ms) /
                              iterations;
...
void bench_bb_gpu_baseline(benchmark::State& state)
{
    if (!cuda_available()) {
...
BENCHMARK(bench_bb_gpu_baseline)
    ->Name("BN254/Baseline/BB_GPU/E2E")
    ->DenseRange(msm_bench::MIN_LOG_NUM_POINTS, msm_bench::MAX_LOG_NUM_POINTS, 2)
    ->Unit(benchmark::kMillisecond);
