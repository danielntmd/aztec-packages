...
msm_bench::Curve::BaseField field_from_raw(const RawField& field)
{
    return msm_bench::Curve::BaseField(
        bb::numeric::uint256_t(field.limbs[0], field.limbs[1], field.limbs[2], field.limbs[3]));
}

msm_bench::Commitment to_cpu_point(const std::array<std::byte, sizeof(msm_bench::Commitment)>& point)
{
    RawAffine raw{};
...
std::array<std::byte, 96> icicle_v28_backend_msm(const RawInput& raw, const uint32_t bits_per_slice)
{
    static_assert(sizeof(RawField) == 32);
    static_assert(sizeof(RawAffine) == 64);
    std::array<std::byte, 96> result{};
    const cudaError_t error =
        bb_gpu_icicle_v28_msm_projective_with_options(raw.scalars.data(),
                                                      raw.points.data(),
                                                      static_cast<int>(raw.scalars.size()),
                                                      static_cast<int>(raw.points.size()),
                                                      static_cast<int>(bits_per_slice),
                                                      false,
                                                      false,
                                                      false,
                                                      false,
...
msm_bench::Commitment finalize_icicle_v28_result(const std::array<std::byte, 96>& projective_result)
{
    std::array<std::byte, sizeof(msm_bench::Commitment)> result{};
    bb_gpu_icicle_v28_projective_to_affine(projective_result.data(), result.data());
    return to_cpu_point(result);
}

msm_bench::Commitment icicle_v28_msm(const msm_bench::BenchInput& input, const uint32_t bits_per_slice)
{
...
DeviceRawInput make_device_raw_input(const RawInput& raw, double& h2d_ms)
{
    ...
    h2d_ms = elapsed_ms([&]() {
        cudaError_t error = cudaMalloc(&device.scalars, raw.scalars.size() * sizeof(RawField));
        if (error != cudaSuccess) {
        ...
        error = cudaMemcpy(
            device.scalars, raw.scalars.data(), raw.scalars.size() * sizeof(RawField), cudaMemcpyHostToDevice);
        if (error != cudaSuccess) {
            fail_cuda(error, "H2D scalars");
        }
        error =
            cudaMemcpy(device.points, raw.points.data(), raw.points.size() * sizeof(RawAffine), cudaMemcpyHostToDevice);
        if (error != cudaSuccess) {
...
void icicle_v28_device_backend_msm(const DeviceRawInput& raw, const uint32_t bits_per_slice)
{
    const cudaError_t error =
        bb_gpu_icicle_v28_msm_projective_with_options(raw.scalars,
                                                      raw.points,
                                                      static_cast<int>(raw.count),
                                                      static_cast<int>(raw.count),
                                                      static_cast<int>(bits_per_slice),
                                                      true,
                                                      false,
                                                      ...
                                                      env_bool("ICICLE_MSM_BIG_TRIANGLE", false),
                                                      env_int("ICICLE_MSM_LARGE_BUCKET_FACTOR", 10),
                                                      raw.projective_result);
    if (error != cudaSuccess) {
        fail_cuda(error, "device-resident MSM");
    }
}

msm_bench::Commitment finalize_icicle_v28_device_result(const DeviceRawInput& raw)
{
    std::array<std::byte, 96> projective_result{};
    if (cudaMemcpy(projective_result.data(), raw.projective_result, projective_result.size(), cudaMemcpyDeviceToHost) !=
        cudaSuccess) {
...
void bench_icicle_v28_e2e(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        ...
        totals.backend_ms += elapsed_ms([&]() { backend_result = icicle_v28_backend_msm(raw, bits_per_slice); });
        totals.postprocess_ms += elapsed_ms([&]() { result = finalize_icicle_v28_result(backend_result); });
...
void bench_icicle_v28_prepared_host(benchmark::State& state)
{
    ...
    auto warmup = finalize_icicle_v28_result(icicle_v28_backend_msm(raw, bits_per_slice));
    benchmark::DoNotOptimize(warmup);
    ...
    add_common_counters(state, bits_per_slice, totals);
    const auto actual = finalize_icicle_v28_result(icicle_v28_backend_msm(raw, bits_per_slice));
    msm_bench::assert_equal("Icicle v2.8 prepared host", log_num_points, expected, actual);
}
...
void bench_icicle_v28_device_resident(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        totals.backend_ms += elapsed_ms([&]() { icicle_v28_device_backend_msm(device, bits_per_slice); });
        benchmark::DoNotOptimize(device.projective_result);
    }

    add_common_counters(state, bits_per_slice, totals);
    const auto actual = finalize_icicle_v28_device_result(device);
    msm_bench::assert_equal("Icicle v2.8 device resident", log_num_points, expected, actual);
}
