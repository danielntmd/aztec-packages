...
uint32_t resolve_bits_per_slice(const size_t num_points, const uint32_t requested_bits)
{
    const uint32_t bits = requested_bits == 0 ? get_auto_bits_per_slice(num_points) : requested_bits;
                                ...
                                const uint32_t requested_bits_per_slice)
{
    ...
    if (points.size() < scalars.end_index()) {
        throw_or_abort("bb::gpu::bn254::msm: point span is smaller than scalar end index");
    }
    if (scalars.end_index() > static_cast<size_t>(std::numeric_limits<uint32_t>::max())) {
        throw_or_abort("bb::gpu::bn254::msm: point indices exceed uint32 schedule range");
    }

    const uint32_t bits_per_slice = resolve_bits_per_slice(scalars.size(), requested_bits_per_slice);
    affine_g1_t result{};
    msm_raw(reinterpret_cast<const fr_t*>(scalars.span.data()),
            scalars.size(),
            reinterpret_cast<const affine_g1_t*>(points.data()),
            points.size(),
            scalars.start_index,
            bits_per_slice,
            &result);
                                                   ...
                                                   bool /*handle_edge_cases*/)
{
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        results.emplace_back(msm({ 0, std::span<const curve::BN254::ScalarField>(scalars[i].data(), scalars[i].size()) },
                                 points[i]));
    }
    return results;
}
...
void shutdown()
{
    // No-op; DeviceContext owns its cached buffers for process lifetime.
}
